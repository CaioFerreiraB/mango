...
import { nomeCategoria, useCategorias, type Categoria } from "@/lib/api/categorias"
import {
...
const SECOES: { tipo: Tipo; titulo: string; vazio: string }[] = [
  { tipo: "despesa", titulo: "Despesas", vazio: "Nenhuma despesa no orçamento padrão ainda." },
  { tipo: "receita", titulo: "Receitas", vazio: "Nenhuma receita no orçamento padrão ainda." },
]
...
export function ConfigurarOrcamentoPadraoDialog({ trigger }: { trigger?: ReactNode } = {}) {
  const [aberto, setAberto] = useState(false)
  const [linhas, setLinhas] = useState<Record<Tipo, Linha[]>>({ despesa: [], receita: [] })
  ...
  function mudarValor(tipo: Tipo, categoriaId: string, centavos: number) {
    setLinhas((prev) => ({
      ...prev,
      [tipo]: prev[tipo].map((l) => (l.categoriaId === categoriaId ? { ...l, valor: centavos } : l)),
    }))
  }
  ...
  function adicionarCategoria(tipo: Tipo, categoriaId: string) {
    setLinhas((prev) => ({
      ...prev,
      [tipo]: [...prev[tipo], { orcamentoId: null, categoriaId, valor: 0, ordem: prev[tipo].length }],
    }))
  }
  ...
  function removerLinha(tipo: Tipo, linha: Linha) {
    if (linha.orcamentoId === null) {
      setLinhas((prev) => ({ ...prev, [tipo]: prev[tipo].filter((l) => l !== linha) }))
      return
    }
  ...
  function arrastarFim(tipo: Tipo, event: DragEndEvent) {
    ...
    setLinhas((prev) => {
      ...
      const reordenada = arrayMove(lista, de, para).map((l, i) => ({ ...l, ordem: i }))
      ...
      for (const linha of linhas[tipo]) {
        ...
        try {
          if (linha.orcamentoId === null) {
            ...
            if (!existente || linha.valor !== existente.limite_padrao_centavos) {
  ...
  function houveMudancas(): boolean {
    for (const { tipo } of SECOES) {
      for (const linha of linhas[tipo]) {
        ...
        const existente = original.get(linha.orcamentoId)
        if (!existente || linha.valor !== existente.limite_padrao_centavos || linha.ordem !== existente.ordem) {
          return true
        }
      }
    }
    const idsAtuais = new Set([...linhas.despesa, ...linhas.receita].map((l) => l.orcamentoId))
    ...
    <Dialog open={aberto} onOpenChange={abrir}>
      ...
      <DialogContent className="sm:max-w-lg">
        <DialogHeader>
          ...
          <DialogDescription>
            Vale a partir do próximo mês materializado — o mês atual não muda automaticamente
            (use "Editar mês" pra isso).
          </DialogDescription>
        </DialogHeader>
        <div className="max-h-[60vh] space-y-6 overflow-y-auto pr-1">
          {SECOES.map(({ tipo, titulo, vazio }) => (
            <SecaoTipo
              ...
              erros={erros}
              onMudarValor={(categoriaId, v) => mudarValor(tipo, categoriaId, v)}
              onRemover={(linha) => removerLinha(tipo, linha)}
              onAdicionar={(categoriaId) => adicionarCategoria(tipo, categoriaId)}
  ...
  return (
    <section className="space-y-2">
      ...
      {linhas.length === 0 ? (
        ...
        <DndContext sensors={sensores} collisionDetection={closestCenter} onDragEnd={onArrastarFim}>
          <SortableContext items={linhas.map((l) => l.categoriaId)} strategy={verticalListSortingStrategy}>
  ...
  return (
    <div ref={setNodeRef} style={style}>
      <div className="flex items-center gap-2 py-1">
        ...
        />
        <Button variant="ghost" size="icon" aria-label={`Remover ${nome}`} onClick={onRemover}>
          <Trash2 className="size-4" />
        </Button>
      </div>
      {erro ? <p className="pb-1 pl-6 text-xs text-destructive">{erro}</p> : null}
    </div>
  )
}
