...
const transacoes: InvestimentoTransacao[] = [
  ...
  }), // 12+ meses → fora da janela; value_unitario null → fallback
  tx({
    id: 4,
    type: "SELL",
...
assert.equal(
  precoCotaCentavos(transacoes[0]!),
  10000,
  "preço via value_unitario"
)
assert.equal(
  precoCotaCentavos(transacoes[2]!),
  10000,
  "preço via fallback amount/qtd"
)
