...
const proventos: InvestimentoTransacao[] = [
  tx({ id: 1, type: "INTEREST", date: "2026-07-14T12:00:00Z", amount_centavos: 1748 }), // real: só total
  tx({ id: 2, type: "INTEREST", date: "2026-06-13T12:00:00Z", amount_centavos: 1748 }),
  tx({ id: 3, type: "DIVIDEND", date: "2025-05-14T12:00:00Z", value_unitario: "0.088", quantity: "1000", amount_centavos: 8800 }), // sandbox: com value/qtd, mas 14 meses atrás → fora do gráfico
]
...
assert.equal(r.ultimoPorCotaReais, null, "último por cota null quando a API não manda value/qtd")
...
assert.equal(r.buckets.reduce((s, b) => s + b.totalCentavos, 0), 3496, "soma das barras = proventos na janela")
assert.equal(r.buckets.find((b) => b.mes === "2026-07")!.totalCentavos, 1748, "bucket do mês corrente")
assert.equal(r.buckets.find((b) => b.mes === "2025-05")!, undefined, "mês -14 não está na janela")

// Tabela: todos os proventos (só mês + total), ordem preservada.
assert.deepEqual(r.linhas.map((l) => l.id), [1, 2, 3], "todas as linhas, ordem preservada")
assert.equal(r.linhas[0]!.totalCentavos, 1748, "total da linha")
assert.notEqual(r.linhas[0]!.label, "—", "label do mês preenchido quando há data")
...
assert.ok(Math.abs(comValor.ultimoPorCotaReais! - 0.088) < 1e-9, "por-cota preserva precisão sub-centavo")
